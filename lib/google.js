...
function getPackageName(receiptData) {
	if (receiptData && receiptData.packageName) {
		return receiptData.packageName;
	} else if (config.googlePackageName) {
		return config.googlePackageName;
	} else {
	  return '';
  }
}
...
function validateProduct(receipt, tokenMap, cb) {
	...
	auth(tokenMap, function (error, body) {
		...
		var url = 'https://www.googleapis.com/androidpublisher/v2/applications/' +
			encodeURIComponent(getPackageName(receipt.data)) +
			'/purchases/products/' +
...
function checkSubscriptionStatus(data, cb) {
	...
	var packageName = getPackageName(data);
