...
module.exports.validatePurchase = function (receipt, cb) {
	...
	if (typeof receipt !== 'object') {
		return cb(new Error('malformed receipt: ' + receipt), { status: constants.VALIDATION.FAILURE, message: 'Malformed receipt' });
	}
	if (!receipt.data || !receipt.signature) {
		return cb(new Error('missing receipt data:\n' + JSON.stringify(receipt)), { status: constants.VALIDATION.FAILURE, message: 'Malformed receipt' });
	}
	...
	validatePublicKey(receipt, getPublicKey(publicKeyMap.live), function (error, data) {
		if (error) {
			...
			validatePublicKey(receipt, getPublicKey(publicKeyMap.sandbox), function (error2, data) {
				...
				// sandbox worked
				checkSubscriptionStatus(data, function(error, response){
					if(error){	
...
function checkSubscriptionStatus(data, cb){
	...
	if(checkSubscriptionState){
		...
		var url = "https://www.googleapis.com/androidpublisher/v2/applications/"+packageName+"/purchases/subscriptions/"+subscriptionID+"/tokens/"+purchaseToken;
		...
		getSubscriptionInfo(url, function(error, response, body){
			...
			if('error' in body)
				return cb(new Error(body.error.message), { status: constants.VALIDATION.FAILURE, message: body.error.message});
...
module.exports.refreshToken = function (cb) {
	...
	if(!checkSubscriptionState){
		return cb(new Error('missing google play api info'), { status: constants.VALIDATION.FAILURE, message: 'client_id, client_secret, access_token and refres_token should be provided' });
	}
	...
	refreshGoogleTokens(function(error, res, body){
		...
		if('error' in parsedBody)
			return cb(new Error(body.error), { status: constants.VALIDATION.FAILURE, message: body.error_description});
