...
module.exports.setup = function (cb) {
	...
	verbose.log('Apple configurations:', config);
...
module.exports.validatePurchase = function (receipt, cb) {
	...
	verbose.log('Validatation data:', content);

	// we try production first
	send('https://' + liveHost + path, content, function (error, res, data) {
		if (error) {

			verbose.log('https://' + liveHost + path, 'failed:', error, data);
		...
		if (data.status === 21007 || data.status === 21002) {
			...
			send('https://' + sandboxHost + path, content, function (error, res, data) {
				if (error) {
					...
					verbose.log('https://' + sandboxHost + path, 'failed:', error, data);
				...
				verbose.log('Sandbox validation successful:', data);
		...
		verbose.log('Production validation successful:', data);
