...
var PRODUCT_VAL = 'https://www.googleapis.com/androidpublisher/v2/applications/%s/purchases/products/%s/tokens/%s?access_token=%s';
var SUBSCR_VAL = 'https://www.googleapis.com/androidpublisher/v2/applications/%s/purchases/subscriptions/%s/tokens/%s?access_token=%s';
...
function validatePurchase(receipt, cb) {
    ...
    _getToken(conf.clientEmail, conf.privateKey, function (error, token) {
        if (error) {
            return cb(error, {
                status: constants.VALIDATION.FAILURE,
...
function _getToken(clientEmail, privateKey, cb) {
    ...
    verbose.log(NAME, 'Get token with', clientEmail, privateKey);
    request(params, function (error, res, body) {
        if (error) {
            return cb(error);
        }
