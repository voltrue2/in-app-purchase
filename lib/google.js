...
module.exports.readConfig = function (configIn) {
	...
	if (configIn && configIn.publicKeyStrSandbox) {
		config.publicKeyStrSandbox = configIn.publicKeyStrSandbox;
	}
	if (configIn && configIn.publicKeyStrLive) {
		config.publicKeyStrLive = configIn.publicKeyStrLive;
	}
...
module.exports.setup = function (cb) {
	if (!config || !config.googlePublicKeyPath) {
		...
		if (config && config.publicKeyStrSandbox) {
			publicKeyMap.sandbox = config.publicKeyStrSandbox;
		}
		if (config && config.publicKeyStrLive) {
			publicKeyMap.live = config.publicKeyStrLive;
		}

		// no google iap or we have the public key value from ENV variables
		return cb();
	}
