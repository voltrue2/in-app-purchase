...
var VALIDATION_PATH = 'https://appstore-sdk.amazon.com/version/' +
	VER + '/verifyReceiptId/developer/' + SECRET + '/user/' + UID + '/purchaseToken/' + PTOKEN;
// currently not used
...
module.exports.readConfig = function (config) {
	...
	VALIDATION_PATH = VALIDATION_PATH.replace('{' + UID + '}', config.uid);
};

module.exports.validatePurchase = function (receipt, cb) {
	var path = VALIDATION_PATH.replace(PTOKEN, receipt);
	send(path, cb);
};
...
module.exports.getPurchaseData = function (purchase) {
	...
	obj.purchaseData = purchase.purchaseData;
...
function send(path, cb) {
	request.get(path, function (error, response, body) {
		var errorMsg = ERRORS.VALIDATION[response.statusCode];
		if (errorMsg) {
			return cb(new Error(errorMsg + ': ' + path));
		}
		if (error) {
			error.message += ': ' + path;
			return cb(error);
