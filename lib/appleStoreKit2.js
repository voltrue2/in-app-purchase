    ...
    try {
        ...
        envsToSetup.forEach(function (env) {
            try {
                ...
                if (appAppleId) {
                    var appleRootCA = '-----BEGIN CERTIFICATE-----\n' +
...
function executeWithFallback(operationName, executeFn, cb) {
    ...
    executeFn(primaryEnv, function (error, result) {
        ...
        if (shouldUseFallback(operationName, primaryEnv, secondaryEnv, error, useFallback)) {
            verbose.log('<AppleStoreKit2>', operationName, 'failed in', primaryEnv, error);
            verbose.log('<AppleStoreKit2>', operationName, 'retrying in', secondaryEnv);
...
function shouldUseFallback(operationName, primaryEnv, secondaryEnv, error, useFallback) {
    if (!useFallback) {
        return false;
    }
    ...
    return operationName === 'validatePurchase' &&
        primaryEnv === Environment.PRODUCTION &&
        secondaryEnv === Environment.SANDBOX &&
...
function isNotFoundError(error) {
    return error.status === 404 ||
        ...
        (error.apiError === 4040001) || // TransactionIdNotFound
        (error.apiError === 4040003) || // OriginalTransactionIdNotFound
        (error.apiError === 4040010);   // TransactionIdNotFoundRetryable
}
...
/**
 * Check if StoreKit 2 is configured
 * @returns {boolean}
 */
module.exports.isConfigured = function () {
    return !!config && enabledEnvironments.length > 0;
};
