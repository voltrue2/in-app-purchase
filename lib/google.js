		...
		validatePublicKey(receipt, getPublicKey(pubkey), function (error, data) {
			if (error) {
				...
				if (isSubscription(receipt)) {
					return validateSubscription(receipt, cb);
				}
...
function validateSubscription(receipt, cb) {
	if (typeof receipt === 'string') {
		try {
			receipt = JSON.parse(receipt);
		} catch (error) {
	...
	verbose.log(NAME, 'Validate purchase as subscription');
	if (!googleTokenMap.clientID || !googleTokenMap.clientSecret || !googleTokenMap.refreshToken) {
	...
	module.exports.refreshToken(function (error, body) {
		if (error) {
			return cb(error, {
				status: constants.VALIDATION.FAILURE,
				message: error.message
		...
		verbose.log(NAME, 'Google API authenticated w/', body);
