...
function getDataFromReceipt(receipt) {
...
module.exports.validatePurchase = function (dPubkey, receipt, cb) {
	...
	if (!getDataFromReceipt(receipt) || !receipt.signature) {
	...
	if (typeof getDataFromReceipt(receipt) === 'object') {
		// stringify and make sure to escpace the value of developerPayload
		receipt.receipt = JSON.stringify(receipt.receipt).replace(/\//g, '\\/');
		verbose.log(NAME, 'Auto stringified receipt data:', getDataFromReceipt(receipt));
	}
...
function validatePublicKey(receipt, pkey, cb) {
	...
	if (valid) {
		...
		var data = JSON.parse(getDataFromReceipt(receipt));
