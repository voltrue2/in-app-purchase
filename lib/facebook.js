...
module.exports.validatePurchase = function (oneTimeSecret, receipt, cb) {
    var appSecret;
    ...
    fb.api(purchaseObj.payment_id + '?', 'get', { fields: 'id,user,actions,application,created_time,country,items,payout_foreign_exchange_rate,phone_support_eligible,refundable_amount,tax,tax_country,test' }, function (paymentRes) {
        if (!paymentRes || paymentRes.error) {
            ...
            cb(new Error('failed to validate purchase: graph api call error: ' + JSON.stringify(notNullPaymentRes.error)), { status: constants.VALIDATION.FAILURE, message: notNullPaymentRes.error });
            return;
        }
